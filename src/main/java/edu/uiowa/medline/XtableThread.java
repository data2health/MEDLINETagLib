...
public class XtableThread implements Runnable {
	...
	public static enum Mode {
		LOAD, MATERIALIZE, REMATERIALIZE
	};
				...
				for (MaterializationRequest theRequest : theRequestVector) {
					try {
						...
						if (mode == Mode.MATERIALIZE) {
							...
							stmt = conn.prepareStatement("insert into medline." + theRequest.tableName + " select " + theRequest.attributeList + " from medline24_staging." + theRequest.tableName);
						} else {
							logger.info("[" + formatter.format(threadID) + "] rematerializing: " + theRequest.tableName + ": " + theRequest.attributeList);
							stmt = conn.prepareStatement("insert into medline." + theRequest.tableName + " select "
									+ theRequest.attributeList + " from medline24_staging." + theRequest.tableName
									+ " where pmid in (select pmid from medline24_staging.queue)");
						}
